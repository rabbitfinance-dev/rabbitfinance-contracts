...
contract RabbitFinance is ERC20, Ownable {
    ...
    bool private _starting;
    uint256 private _startBlock;
    uint256 supply = 100 * 1e6 * 1e18;
    uint256 transferTaxAt = supply * 5 / 10000;
    uint256 public buyTax = 3;
    uint256 public sellTax = 5;

    mapping(address => bool) private _isExcludedFromTax;
    event ExcludeFromTax(address indexed account, bool isFreeTax);

    constructor(IRouter router_, address marketing_) ERC20("RabbitFinance", "RabbitF") {
        router = router_;
    ...
    function excludeFromTax(address account, bool isExcluded) public onlyOwner {
        ...
        emit ExcludeFromTax(account, isExcluded);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (_startBlock == 0 && to == swapPair) {
            _starting = true;
            _startBlock = block.number;
        } else if (_starting == true && block.number > (_startBlock + 20)) {
            buyTax = 3;
            sellTax = 5;
            _starting = false;
        }
